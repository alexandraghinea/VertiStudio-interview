  ...
  `);
	...
	db.transaction(() => {
		for (let i = 0; i < count; i++) {
			...
			const password_hash = createHash('sha256').update('password123').digest('hex'); // Default password for seeded users

			const info = insertClient.run(name, email, password_hash, phone, address);
  ...
  `);
	...
	db.transaction(() => {
		for (let i = 0; i < count; i++) {
			...
			// Get BitSlow value
			const coinValue =
				db.query("SELECT value FROM coins WHERE coin_id = ?").get(coinId) as { value: number } | null;

			// Use exact BitSlow value as the transaction amount
			const amount = coinValue?.value || 0;
